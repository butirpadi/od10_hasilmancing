...
class hm_wiz_gaji_driver(models.TransientModel):
	_name = 'hm_wiz_gaji_driver'
	_description = 'Wizard Gaji Driver'

	name = fields.Char('name', default="Report Gaji Driver")
	...
	payroll_ids = fields.Many2many('hm_payroll_driver',string='Gaji Driver')
	...
	def do_submit_report(self):
		...
		# payroll get
		query = ('select * from hm_payroll_driver where tanggal >= %s and tanggal <= %s '
					+ ' and ' + by_karyawan  
					+ ' and ' + by_pay_state  
